...
def verify_ledger(records: list[dict]) -> IntegrityReport:
    ...
    for record in records:
        ...
        if record["prev_hash"] != expected_prev:
            forks.append(seq)
        if record.get("kind") == "commitment" and _reseal(record) != record["seal"]:
            broken.append(record["commitment_id"])
        previous_seq = seq
        expected_prev = digest(record)

    intact = not (gaps or forks or broken)
    return IntegrityReport(intact=intact, gaps=gaps, forks=forks, broken_seals=broken)
